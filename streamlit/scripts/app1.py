...
def generate_diagram(
    ...
    attempt = 1
    graph_error = True
    # with st.status("Generating visual gist (attempt " + str(attempt) + ")", expanded=True):
    while graph_error == True:
        st.write("Generating visual gist (attempt " + str(attempt) + ")")
...
def text_url_changed():
    ...
    elif st.session_state.text_url == " ":
        st.session_state.remaining_graphs = 0
    else:
...
if 'prompt_template' not in st.session_state:
    ...
    4. The orientation of the Mermaid {kind} should be {orientation}.
